...
class ExnessAdapter extends BrokerAdapter {
    constructor(config) {
        super(config);
        this.apiUrl = config.apiUrl || process.env.EXNESS_API_URL || 'https://api.exness.com';
    ...
    async login() {
        try {
            const response = await axios.post(`${this.apiUrl}/v1/auth/login`, {
                ...
                server: this.server || 'Exness-Demo',
            });
            this.sessionId = response.data.sessionId;
            this.accessToken = response.data.token;
            this.loggedIn = true;
            console.log(`✅ Exness: Logged in successfully (Account: ${this.accountId})`);
            return true;
        } catch (error) {
            console.error('Exness login error:', error.response?.data || error.message);
            this.loggedIn = false;
            return false;
        }
    }
    ...
    async getBalance() {
        ...
        try {
            ...
            console.error('Exness balance error:', error.response?.data || error.message);
            return null;
        }
    }
    ...
    async getQuote(symbol) {
        ...
        try {
            ...
            console.error('Exness quote error:', error.response?.data || error.message);
            return null;
        }
    }
    ...
    async placeOrder(symbol, direction, volume, stopLoss, takeProfit) {
        ...
        try {
            ...
            console.error('Exness order error:', error.response?.data || error.message);
            return {
                success: false,
    ...
    async getPositions() {
        ...
        try {
            ...
            console.error('Exness positions error:', error.response?.data || error.message);
            return [];
        }
    }
    ...
    async closePosition(positionId) {
        ...
        try {
            ...
            console.error('Exness close error:', error.response?.data || error.message);
            return {
                success: false,
    ...
    async modifyStopLoss(positionId, stopLoss) {
        ...
        try {
            ...
            console.error('Exness modify SL error:', error.response?.data || error.message);
            return {
                success: false,
    ...
    async getOpenOrders() {
        ...
        try {
            ...
            console.error('Exness orders error:', error.response?.data || error.message);
            return [];
        }
    }
}

module.exports = { ExnessAdapter };
