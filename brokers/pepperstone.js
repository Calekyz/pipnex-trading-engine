...
class PepperstoneAdapter extends BrokerAdapter {
    constructor(config) {
        super(config);
        this.apiUrl = config.apiUrl || process.env.PEPPERSTONE_API_URL || 'https://api.pepperstone.com';
    ...
    async login() {
        try {
            const response = await axios.post(`${this.apiUrl}/v1/auth/login`, {
                ...
                server: this.server || 'Pepperstone-Demo',
            });
            this.sessionId = response.data.sessionId;
            this.accessToken = response.data.token;
            this.loggedIn = true;
            console.log(`✅ Pepperstone: Logged in successfully (Account: ${this.accountId})`);
            return true;
        } catch (error) {
            console.error('Pepperstone login error:', error.response?.data || error.message);
            this.loggedIn = false;
            return false;
        }
    }
    ...
    async getBalance() {
        ...
        try {
            ...
            console.error('Pepperstone balance error:', error.response?.data || error.message);
            return null;
        }
    }
    ...
    async getQuote(symbol) {
        ...
        try {
            ...
            console.error('Pepperstone quote error:', error.response?.data || error.message);
            return null;
        }
    }
    ...
    async placeOrder(symbol, direction, volume, stopLoss, takeProfit) {
        ...
        try {
            ...
            console.error('Pepperstone order error:', error.response?.data || error.message);
            return {
                success: false,
    ...
    async getPositions() {
        ...
        try {
            ...
            console.error('Pepperstone positions error:', error.response?.data || error.message);
            return [];
        }
    }
    ...
    async closePosition(positionId) {
        ...
        try {
            ...
            console.error('Pepperstone close error:', error.response?.data || error.message);
            return {
                success: false,
    ...
    async modifyStopLoss(positionId, stopLoss) {
        ...
        try {
            ...
            console.error('Pepperstone modify SL error:', error.response?.data || error.message);
            return {
                success: false,
    ...
    async getOpenOrders() {
        ...
        try {
            ...
            console.error('Pepperstone orders error:', error.response?.data || error.message);
            return [];
        }
    }
}

module.exports = { PepperstoneAdapter };
